...
def compute(opcode, parameters, codeinput, usercodeinput):
    global currpos
    # print("Got a", opcode, "at position", currpos)
    ...
    if opcode == 5:
        ...
        if int1 != 0:
            ...
            currpos += 3
    if opcode == 6:
        # print("opcode 6 int 1", int1)
        if int1 == 0:
            currpos = int2
        else:
            currpos += 3
    if opcode == 7:
        # print("opcode 7 int 1 and 2", int1, int2)
        if int1 < int2:
            codeinput[codeinput[currpos+3]] = 1
        else:
            codeinput[codeinput[currpos+3]] = 0
        currpos += 4
    if opcode == 8:
        # print("opcode 8 int 1 and 2", int1, int2)
        if int1 == int2:
            codeinput[codeinput[currpos+3]] = 1
        else:
            codeinput[codeinput[currpos+3]] = 0
