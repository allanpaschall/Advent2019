...
namespace AoC2019
{
    public class Day11
    {
        ...
        public void Run()
        {
            ...
            Console.WriteLine("Day 11,P1:" + (from p in RunPainter(0) where p != 0 select p).Count() + ", completed in " + (System.DateTime.Now - begin).TotalMilliseconds + " milliseconds");
            begin = DateTime.Now;

            var p2 = RunPainter(1);
            int[] Input = new int[6 * 5 * 8];
            for (int x = 95; x <= 100; x++)
            {
                for (int y = 101; y < 141; y++)
                {
                    var ix = 5-(x - 95);
                    var iy = y - 101;
                    Input[iy + ix*40] = p2[y*200+x];
                }
            }
            //for (int i = 0; i < 6; i++)
            //{
            //    for (int j = 0; j < 40; j++)
            //    {
            //        Console.Write(Input[i * 40 + j] == 2 ? '#' : ' ');
            //    }
            //    Console.WriteLine();
            //}
            Console.WriteLine("Day 11,P2:" + CharacterOCR.ReturnCharacters(Input, 8, 2) + ", completed in " + (System.DateTime.Now - begin).TotalMilliseconds + " milliseconds");

        }

        public int[] RunPainter(int initial)
        {
            ...
            i.Input.Enqueue(initial);
            bool result = true;
            do
            {
                result = i.Run(RunningMode.OutputAttached);
                if (i.Output.Count != 0)
                {
                    var p = i.Output.Dequeue();
                    //Console.WriteLine("Paint:" + p);
                    map[y*200+ x] = (int)p + 1;
                }

                //get 2 outputs, then provide 1 input.
                if (result == false)
                    result = i.Run(RunningMode.OutputAttached);
                if (i.Output.Count != 0)
                {
                    var t = i.Output.Dequeue();
                    ...
                    i.Input.Enqueue(map[y*200+ x] == 2 ? 1 : 0);
                }
            } while (!result);
            return map;
