...
namespace AoC2019
{
    ...
    public class Day17
    {
        ...
        public void Run()
        {
            ...
            Dictionary<int, char> points = new Dictionary<int, char>();
            var i = new IntCode(Data,ProvideInput);
            i.Run();
            int x = 0;
            int y = 0;
            int count = i.Output.Count;
            for (int j = 0; j < count; j++)
            {
                var type = (char)i.Output.Dequeue();
                if (type=='\n')
                {
                    y++ ;
                    x = 0;
            ...
            for (int yy = 1; yy < 100; yy++)
            {
                ...
                {
                    if (points.ContainsKey(yy*100+xx) && points[yy*100+xx]=='#')
                    {
                        //check up
                        if ((points.ContainsKey((yy-1)*100+xx) && points[(yy-1)*100+xx]=='#') &&
                        //check down
                        (points.ContainsKey((yy + 1) * 100 + xx) && points[(yy + 1) * 100 + xx] == '#') &&
                        //check left
                        (points.ContainsKey((yy) * 100 + xx-1) && points[(yy) * 100 + xx-1] == '#') &&
                        //check right
                        (points.ContainsKey((yy) * 100 + xx+1) && points[(yy) * 100 + xx+1] == '#'))
                        {
                            //get the x and y and store...
                            output += yy * xx;
                        }
            ...
            Console.WriteLine("Day 17,P1:" + output + ", completed in " + (System.DateTime.Now - begin).TotalMilliseconds + " milliseconds");
